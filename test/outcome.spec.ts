...
const outcome = new HandOutcome(HandResult.BancoWins, 20, 30, [ace, two], [ace, two], 0, 3)
...
describe('handoutcome.ts', ()=> {
	...
	test('getPayout.banker.lose', ()=> {
		const bet = new Bet(new BancoMun(), 100)
		const outcome = new HandOutcome(HandResult.PuntoWins, 20, 30, [ace, two], [ace, two], 0, 3)
		const payout = HandOutcome.getPayout(bet, outcome, config)
		expect(payout).toBe(0)
	})
	...
	test('getPayout.banker.6', ()=> {
		...
		const outcome = new HandOutcome(HandResult.BancoWins, 20, 30, bhand, [ace, two], 0, 3)
		const payout = HandOutcome.getPayout(bet, outcome, config)
	...
	test('getPayout.player.tie', ()=> {
		const bet = new Bet(new PuntoMun(), 100)
		const outcome = new HandOutcome(HandResult.Tie, 20, 30, [ace, two], [ace, two], 0, 3)
		const payout = HandOutcome.getPayout(bet, outcome, config)
